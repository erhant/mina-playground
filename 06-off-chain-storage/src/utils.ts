...
export const makeAndSendTransaction = async <State extends ToString>({
  ...
  let transaction = await Mina.transaction({ feePayerKey: feePayerPrivateKey, fee: transactionFee }, () => {
    mutateZkApp();
  });
...
export const zkAppNeedsInitialization = async ({ zkAppAccount }: { zkAppAccount: FetchedAccount }) => {
  console.warn(
  ...
  const allZeros = zkAppAccount.appState!.every((f: Field) => f.equals(Field.zero).toBoolean());
